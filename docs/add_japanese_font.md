...
| `jp_stroke_width`                    | 日本語 glyph だけを FontForge stroke で太らせる幅。元フォントに中間weightがなく、英字より細く見える場合に小さい値から試す。`0` なら補正しない                                                                |
...
| 上記以外で合成元の送り幅が全角EMに近い文字 | `jp_width` に設定 |
...
| ------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------- |
| 全角かな・漢字      | `あ 漢 日 が ぱ`              | 大きさと太さのバランス (`jp_scale_offset` / `jp_stroke_width`)、濁点・半濁点の潰れ             |
