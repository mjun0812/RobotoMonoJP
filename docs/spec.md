...
1つのconfigから通常版とConsole向け`-Mono`版の2 familyを生成する。

- RobotoMono由来の `WIDTH=1299` を維持。
- 半角=1299、全角=1849 (1.42:1)。
- 日本語の見た目重視、緩めの等幅。

通常版と`-Mono`版はそれぞれ`Regular` / `Bold` / `Italic` / `BoldItalic`の4スタイルを持つ。
合計で2 family × 4 style × (ttf, otf) = 16ファイル。

通常版は合成元フォントの曖昧幅文字を全角幅にし、`-Mono`版は曖昧幅文字を半角セルへ縮小する。

## 命名規則

JP Fontを差し替え可能にするため、`familyname` は `jp_identifier` から機械的に組み立てる。

- `RobotoMono{jp_identifier}` (通常版)
- `RobotoMono{jp_identifier}-Mono` (Console向け)
...
`-Mono`版はConsole向けの曖昧幅記号を半角セルに収めるfamilyとして通常版と同時に生成する。
...
Font作成endpoint。1回の呼び出しで通常版と`-Mono`版の16ファイルを一括生成する。
...
2. JPスケール適用: 旧 `main.py` と同じく `ascent / 元JPフォントのascent + jp_scale_offset` 倍に拡縮し、半角カナ幅を `en_width`、全角幅を `jp_width` に合わせる。かな・漢字以外の記号は合成元フォントの送り幅を基準に、通常版では全角EMに近ければ `jp_width`、半角EMに近ければ `en_width` に合わせる。Mono版ではEast Asian Widthが`A`の文字を`en_width`へ縮小する。
...
出力先は `dist/{familyname}/{familyname}-{style}.(ttf|otf)` と`dist/{familyname}-Mono/{familyname}-Mono-{style}.(ttf|otf)`で固定。
...
4. `gh release create` で全config分のfont (config数 × 2 family × 4 style × (ttf, otf)) をindividual assetとしてuploadする。
